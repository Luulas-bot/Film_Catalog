...
class NewMovie():
    ...
    def init_stats(self):
        ...
        self.error_abrevation_text = self.font2.render("Error! La abreviación del país o del género es incorrecta.", True, GOLD)
        self.error_name_text = self.font2.render("Error! La película debe tener un nombre.", True, GOLD)

        # Índice de las lines de las descripcion
        self.desc_index = 0
        ...
        self.error_abreviation_state = False
        ...
        # NmText texts y NmDescription texts
        ...
        # Listas que contienen parcialmente o totalmente los objetos definidos arriba
        self.description_list = []
        self.texts_list = []

        # Se añaden a una lista los objetos definidos arriba de la descripción
        for i in NmDescription.description_list_temp:
            self.description_list.append(i)

        # Se añaden a una lista todos los objetos definidos arriba excepto aquellos de la descripción
    ...
    def draw_on_screen(self):
        ...
        # Muestra por pantalla los títulos
        ...
        if self.error_abreviation_state == True:
            self.screen.blit(self.error_abrevation_text, (50, 557))
        elif self.error_name_state == True:
            self.screen.blit(self.error_name_text, (50, 557))
        ...
        if self.event.type == pygame.KEYDOWN:
            if self.event.key == pygame.K_TAB:
                if self.name_text.state:    
                    ...
                    self.description_list[self.desc_index].text = self.description_list[self.desc_index].text[:-1]
                    self.description_text.state = False
    ...
    # Mecánicas del enter (el pass del final sirve para solucionar un bug)
            ...
            if self.event.type == pygame.KEYDOWN:
                if self.event.key == pygame.K_RETURN:
                    if i.state and len(i.text) != 0:
                        i.text = i.text[:-1]
                    elif self.description_text.state and len(self.description_list[self.desc_index].text) != 0 and self.desc_index < 14:
                        self.desc_index += 1
                        self.description_list[self.desc_index - 1].text = self.description_list[self.desc_index - 1].text[:-1]
                    elif self.desc_index == 15:
                        pass
                    
    # Mecánicas del tick (inserción de los datos en la bas de datos)
    def get_tick(self):
        try:    
            if self.event.type == pygame.MOUSEBUTTONDOWN:
                if self.tick_rect.collidepoint(self.mx, self.my):
                    if len(self.name_text.text) != 0:    
                        ...
                        self.error_abrevation_state = False    
        except IntegrityError:
            self.error_abrevation_state = True
            self.error_name_state = False
    ...
    def description_display(self): 
        for i in self.description_list:   
            self.desc_surface = self.font2.render(i.text, True, (47, 86, 233))
            self.screen.blit(self.desc_surface, (i.blit_coords))
    ...
    def description_mechanics(self):    
        if self.event.key == pygame.K_BACKSPACE and self.description_text.state:
            self.description_list[self.desc_index].text = self.description_list[self.desc_index].text[:-1]
            if self.desc_index < 15 and len(self.description_list[self.desc_index].text) == 0 and self.desc_index != 0:
                self.desc_index -= 1
        elif self.description_text.state and len(self.description_list[self.desc_index].text) < 61:
            self.description_list[self.desc_index].text += self.event.unicode
        elif len(self.description_list[self.desc_index].text) > 59 and self.desc_index < 15 and self.description_text.state:
            self.desc_index += 1
